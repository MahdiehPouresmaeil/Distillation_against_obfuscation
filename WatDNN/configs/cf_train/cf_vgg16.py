...
cf_vgg16_dict = {"batch_size": batch_size, "lr": 1e-3, "epochs": epochs, "wd": 1e-4, "opt": "Adam",
                    "architecture": architecture, "milestones": [150, 200], "gamma": 0.1, "criterion": criterion,
                    "scheduler": "CosineAnnealingLR",
                    "device": device,
                    "database": database,
                    "momentum": 0.9,
                    "save_path": "results/trained_models/vgg16/_db" + database + "_ep" + str(epochs) +
                                 "_bs" + str(batch_size) + ".pth",
                    "save_fig_path": "results/trained_models/vgg16/_db" + database
                                     + "_ep" + str(epochs) + "_bs" + str(batch_size),
                    "show_acc_epoch": True,
                    # "lambda": 1e-2 #for test

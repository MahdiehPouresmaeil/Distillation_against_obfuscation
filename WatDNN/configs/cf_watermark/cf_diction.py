...
from configs.cf_train.cf_cnn import cf_cnn_dict, cf_cnn2_dict
from configs.cf_train.cf_mlp_riga import cf_mlp_riga_dict
from configs.cf_train.cf_resnet18 import cf_resnet18_dict
from configs.cf_train.cf_VAE_model import cf_vae_dict

from copy import deepcopy

from configs.cf_train.cf_vgg16 import cf_vgg16_dict
...
cf_mlp_embed = {"method_wat": method_wat,
                "configuration": cf_mlp_dict,  # the configuration of the target model
...
watermark_size = 256
epochs_embed = 10000
layer_name = ["conv4"]  # fc1
epoch_check =30
save_path_embed = "11012026_l" + layer_name[0] + "_wat" + str(watermark_size) + "_ep" + str(epochs_embed) + "_epc" + \
                  str(epoch_check)
...
epoch_check = 10
watermark_size = 256
save_path_attack = "normalized_activation_l" + layer_name[0] + "_wat" + str(watermark_size) + "_ep" + str(epoch_attack) \
                   + "_epc" + str(epoch_check)
...
epochs_embed=100
epoch_check=30
layer_name=cf_cnn_embed["layer_name"]
save_path_attack = "l_" + str(layer_name) + "_attack_type_" + str(attack_type)
cf_cnn_attack_ambiguity= {"method_wat": method_wat,
                "configuration": cf_cnn_dict,
                "database": cf_cnn_dict["database"],
                "watermark_size": watermark_size,
                "epochs": epochs_embed,
                ...
                "batch_size": int(cf_cnn_dict["batch_size"] * 1.),
                "mean": 0,   #0,0,0.5,0,
                "std": 1, #1,0.1,0.5,0.05,
                #N(0, 0.1) - Small variance around zero ,  N(0.5, 0.5) - Centered around 0.5, N(0, 0.05) light noise
                ...
                "path_model": cf_cnn_dict["save_path"],
                "path_model_fake":cf_cnn2_dict["save_path"],
                "save_path": "results/watermarked_models/" + method_wat + "/" + cf_cnn_dict['architecture'].lower() + "/"
                             + save_path_embed + ".pth",

                "save_path_orig": "results/attacks/ambiguity/" + method_wat + "/" + cf_cnn_dict['architecture'].lower() + "/"
                             +"finetuned_original_fake_ep50_"+ save_path_embed + ".pth",

                "save_path_wat": "results/attacks/ambiguity/" + method_wat + "/" + cf_cnn_dict['architecture'].lower() + "/"
                             +"wat_attacked_model_ep50_2"+ save_path_embed + ".pth",
...
attack_type="logits"
epoch_attack =1000
save_path_attack = "_l_" + layer_name + "_attack_type_" + str(attack_type) + "_epochs_" + str(epoch_attack)
...
cf_cnn_attack_distillation = {
    ...
    "epoch_attack": epoch_attack,
    # "path_student":cf_cnn_dict["save_path"],
    # "path_fine_tuned":cf_cnn_attack_ft["save_path"],
    # "path_ambiguity":cf_cnn_attack_ambiguity["save_path_wat"]
...
# layer_name=["base.layer3.0.conv2"]
layer_name=["base.layer4.1.conv2"]
...
# layer_name = ["base.fc.0"]
# layer_name=["base.layer3.0.conv2"]
# layer_name=["base.layer2.0.conv2"]
epoch_attack = 100
watermark_size = 256
epoch_check = 20
save_path_attack = "_l" + layer_name[0] + "_wat" + str(watermark_size) + "_ep" + str(epoch_attack) \
...
cf_mlp_riga_embed = {"method_wat": method_wat,
...
#********************************************************************
#************** watermarking VAE architecture ****************
#********************************************************************

watermark_size = 256
epochs_embed = 10000
layer_name = ["decoder.conv2"]  # fc1
epoch_check = 30
save_path_embed = "_l" + layer_name[0] + "_wat" + str(watermark_size) + "_ep" + str(epochs_embed) + "_epc" + \
                  str(epoch_check)

cf_vae_embed={ "model": 'VAE',
                "method_wat": method_wat,
                "configuration": cf_vae_dict,  # the configuration of the target model
                "database": cf_vae_dict["database"],  # the database to train the target model
                "watermark_size": watermark_size,  # the size of the watermark to be embedded
                "epochs": epochs_embed,  # the maximum number of epochs to train the target model
                "epoch_check": epoch_check,  # the number of epochs to check the watermarking process

                # Latent space parameters
                "batch_size": int(cf_vae_dict["batch_size"] * 1.),  # the number of samples of trigger set in a batch
                ...
                "opt": "Adam",  # optimizer
                "scheduler": "MultiStepLR",  # the scheduler to update the learning rate
                "architecture": cf_vae_dict["architecture"],  # the architecture of the target model
                "momentum": 0,  # momentum to train the target model (if needed)
                "milestones": [20, 60],  # the milestones to update the learning rate
                "gamma": 0.1,  # the factor to update the learning rate
                "criterion": cf_vae_dict["criterion"],  # the criterion to train the target model
                "device": cf_vae_dict["device"],  # the device to train the target model

                "path_model": cf_vae_dict["save_path"],  # Path of the original model
                "save_path": "results/watermarked_models/" + method_wat + "/" + cf_vae_dict['architecture'].lower() + "/"
                             + save_path_embed + ".pth"  # Path to save the target/watermarked model
}
cf_vae_attack_ft={}
cf_vae_attack_pr={}
cf_vae_attack_ow={}
cf_vae_attack_pia={}
cf_vae_attack_dummy_neurons={}
cf_vae_attack_distillation={}
cf_vae_attack_ambiguity={}


#********************************************************************
#************** watermarking VGG16 architecture ******************
#********************************************************************
watermark_size = 256
epochs_embed = 1000
epoch_check =10
...
# layer_name = ["base.fc.0"]
# layer_name=["base.layer3.0.conv2"]
layer_name=["base.features.28"]
# layer1.0.conv1', layer3.1.conv1, view
save_path_embed = "_l" + layer_name[0] + "_wat" + str(watermark_size) + "_ep" + str(epochs_embed) + "_epc" + \
                  str(epoch_check)
cf_vgg16_embed = {"method_wat": method_wat,
                    "configuration": cf_vgg16_dict,
                     "database": cf_vgg16_dict["database"],
                     "watermark_size": watermark_size,
                     "epochs": epochs_embed,
                     "epoch_check": epoch_check,
                     # Latent space parameters
                     ...
                     "batch_size": int(cf_vgg16_dict["batch_size"] * 1.),
                     "n_features": 1.,
                     ...
                     "opt": "Adam",
                     "scheduler": "MultiStepLR",
                     "architecture": cf_vgg16_dict['architecture'],
                     "save_path": "results/watermarked_models/" + method_wat + "/" + cf_vgg16_dict[
                         'architecture'].lower() + "/" + save_path_embed + ".pth",
                     # relu but change in mlp cd
                     "path_model": cf_vgg16_dict["save_path"],
                     "momentum": 0,
                     "milestones": [20, 150],
                     "gamma": .1,
                     "criterion": cf_vgg16_dict["criterion"],
                     "device": cf_vgg16_dict["device"]
                     }
...
save_path_attack = "_l" + layer_name[0] + "_wat" + str(watermark_size) + "_ep" + str(epoch_attack) + "_epc" + \
                   str(epoch_check)
cf_vgg16_attack_ft = {"configuration": cf_resnet18_dict,
                         "database": cf_resnet18_dict["database"],
                         "path_model": cf_resnet18_embed['save_path'] + '.pth',
                         "epochs": epoch_attack,
                         "watermark_size": watermark_size,
...
#### *********************************Pruning attack*****************************************
cf_vgg16_attack_pr = {"configuration": cf_resnet18_dict,
                         "database": cf_resnet18_dict["database"],
                         "path_model": cf_resnet18_embed["save_path"] + ".pth",
                         "watermark_size": watermark_size,
                         "criterion": cf_resnet18_dict["criterion"],
...
save_path_attack = "_l" + layer_name[0] + "_wat" + str(watermark_size) + "_ep" + str(epoch_attack) \
                   + "_epc" + str(epoch_check)
cf_vgg16_attack_ow = {"configuration": cf_resnet18_dict,
                         "database": cf_resnet18_dict["database"],
                         "path_model": cf_resnet18_embed["save_path"] + ".pth",
                         "watermark_size": watermark_size,
                         # Latent space parameters
...
#### *********************************PIA attack*********************************************
cf_vgg16_attack_pia = {"train_non_watermarked": cf_non_watermarked,
                          "train_watermarked": cf_watermarked,
                          "save_path": "results/attacks/pia/" + method_wat + "/" + cf_resnet18_dict[
                              "architecture"].lower() + "/" + save_path_attack + ".pth",
                          "nb_examples": nb_examples
...
attack_type="neuron_clique"
save_path_attack = "l_" + layer_name + "_attack_type_" + str(attack_type)
cf_vgg16_attack_dummy_neurons = {
    "configuration": cf_vgg16_dict,
    "database": cf_vgg16_dict["database"],
    "path_model": cf_vgg16_embed["save_path"],
    "save_path": "results/attacks/dummy_neurons/" + method_wat + "/" + cf_vgg16_dict[
        "architecture"].lower() + "/" + save_path_attack + ".pth",
    "device":cf_vgg16_dict["device"],
    "attack_type":attack_type,
    "layer_name": layer_name,
...
cf_vgg16_attack_distillation = {
    "configuration": cf_vgg16_dict,
    "database": cf_vgg16_dict["database"],
    "path_model": cf_vgg16_attack_dummy_neurons["save_path"],
    "save_path": "results/attacks/distillation/" + method_wat + "/" + cf_vgg16_dict[
        "architecture"].lower() + "/" + save_path_attack + ".pth",
    "device":cf_vgg16_dict["device"],
    "attack_type":attack_type,
    "layer_name": layer_name,

...
epochs_embed = 10
epoch_check = 5
layer_name = ['conv1', 'layer1.0.conv1', 'layer1.0.conv2']
save_path_embed = "_l" + layer_name[0] + "_ep" + str(epochs_embed) + "_epc" + \
                  str(epoch_check)
...
#### *********************************Fine tuning attack************************************
epoch_attack = 2
save_path_attack = "_l" + layer_name[0] + "_ep" + str(epoch_attack) + ".pth"
...
epoch_check = 2
layer_name = ["fc1"]
save_path_attack = "_l" + layer_name[0] + "_ep" + str(epoch_attack) \
...
nb_examples = 800
save_path_attack = "_l" + layer_name[0] + "_ep" + str(cf_non_watermarked["epochs"]) + "_nb_examples" + str(nb_examples)
...
# watermark_size = Passport activation shape: torch.Size([1, 64, 32, 32])   #depends on the activation shape of the layer here in cnn conve 4 is  64
epochs_embed = 1000
...
cf_resnet18_embed = {"method_wat": method_wat,
                "configuration": cf_resnet18_dict,  # the configuration of the target model
                "database": cf_resnet18_dict["database"],  # the database to train the target model
                "epochs": epochs_embed,  # the maximum number of epochs to train the target model
                "epoch_check": epoch_check,  # the number of epochs to check the watermarking process

                # Latent space parameters
                "batch_size": int(cf_resnet18_dict["batch_size"] * 1.),  # the number of samples of trigger set in a batch
                ...
                "opt": "Adam",  # optimizer
                "scheduler": "MultiStepLR",  # the scheduler to update the learning rate
                "architecture": cf_resnet18_dict["architecture"],  # the architecture of the target model
                "momentum": 0,  # momentum to train the target model (if needed)
                "milestones": [20, 60],  # the milestones to update the learning rate
                "gamma": 0.1,  # the factor to update the learning rate
                "criterion": cf_resnet18_dict["criterion"],  # the criterion to train the target model
                "device": cf_resnet18_dict["device"],  # the device to train the target model

                "path_model": cf_resnet18_dict["save_path"],  # Path of the original model
                "save_path": "results/watermarked_models/" + method_wat + "/" + cf_resnet18_dict['architecture'].lower() + "/"
                             + save_path_embed + ".pth"  # Path to save the target/watermarked model
...
#### *********************************Fine tuning attack*************************************
epoch_attack = 10
save_path_attack = "_l" + layer_name[0] + "_ep" + str(epoch_attack) + "_epc" + \
                   str(epoch_check)
cf_resnet18_attack_ft = {

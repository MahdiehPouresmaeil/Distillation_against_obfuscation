...
class Deep_Signs(Watermark):
    ...
    def keygen(self, watermark_size, nb_wat_classes, n_features):
        """
        Generate a secret key and message.
        """
        # Generate the watermark
        watermark = 1. * torch.randint(0, 2, size=(nb_wat_classes, watermark_size))

        # generate matrix matrix_a
        matrix_a = 1. * torch.randn(n_features, watermark_size, requires_grad=False).cuda()

        return watermark, matrix_a

    # Computing loss1
    @staticmethod
    def _mu_loss1(act, mu, mu_bar, watermarked_classes, y_key):
    ...
    @staticmethod
    def _get_trigger_set(gmm, act, x_train, watermarked_classes, percent):
        ...
        if nb_samplers < indices_gmm_train.shape[0]:
            print("nb_samplers < indices_gmm_train.shape[0]", nb_samplers, indices_gmm_train.shape[0])
            ...
            print("x_key size1 = ", len(key_index))
        else:
            x_key, y_key = x_train_tmp[indices_gmm_train], y_train_tmp[indices_gmm_train]
            print("x_key size2 = ", len(indices_gmm_train))

        return x_key, y_key

    @staticmethod
    def _process_activation(act):
        """
        Process activation maps based on architecture type.
        For CNNs: use global average pooling (as in DeepSigns paper)
        For MLPs: use as is
        """
        if len(act.shape) == 4:  # CNN case: (batch, channels, height, width)
            # Global Average Pooling - recommended in DeepSigns paper
            # This reduces spatial dimensions while preserving channel information
            act = torch.mean(act, dim=(2, 3))  # Results in (batch, channels)

        elif len(act.shape) == 3:  # Possible case for some architectures
            act = act.view(act.shape[0], -1)

        # If len(act.shape) == 2, it's already in the right format (MLP case)
    ...
    def embed(self, init_model, test_loader, train_loader, config) -> object:
        ...
        train_nodes, eval_nodes = get_graph_node_names(init_model)
        print(eval_nodes)
        ...
        for images, label in train_loader:
            ...
            act_batch = extractor(images)[config["layer_name"]].detach().cpu()
            # Process activation based on architecture
            act_batch = self._process_activation(act_batch)
            act.append(act_batch)

        print("act shape after processing:", act[0].shape)
        train_data = torch.cat(train_data).cuda()
        train_labels = torch.cat(train_labels).cuda()
        act = torch.cat(act).cuda()

        config["n_features"] = act.shape[1] if len(act.shape) == 2 else len(act[0])

        print("config n_features ", config["n_features"])
        print("train_data.shape: ", train_data.shape, "train_labels.shape: ", train_labels.shape, "act.shape: ",
              act.shape)

        # Now get the gmm to generate the trigger set
        gmm, _, _, gmm_nonzero = self._gmm(act, config)
        # T in the paper
        ...
        # generate the trigger set
        x_key, y_key = self._get_trigger_set(gmm=gmm, act=act, x_train=train_data,
        ...
        # Generate the watermark and the secret matrix A
        watermark, matrix_a = self.keygen(config["watermark_size"], config["nb_wat_classes"], config["n_features"])
        ...
        mu = gmm.mu.squeeze(0)    #mu fpr all classes
        # print("mu shape ", mu.shape)
        t_bar = [i for i in range(config["n_components"]) if i not in watermarked_classes]
        mu_bar = mu[t_bar]   #mu for non watermarked classes
        # print("mu_bar shape ", mu_bar.shape)
        ...
        for epoch in range(config["epochs"]):
            ...
            for batch_idx, (x_train, y_train) in enumerate(loop):
                ...
                # Process activation based on architecture
                act = self._process_activation(act)
                ...
                gmm_loss, mu_loss = self._mu_loss1(act, mu_dp, mu_bar, watermarked_classes, y_key_)
                # sanity check
                assert gmm_loss.requires_grad == True, 'broken computational graph :/'
                matrix_g = model_deepSigns(matrix_a)

                assert BCELoss(reduction='sum')(matrix_g,
                                                watermark.cuda()).requires_grad == True, 'broken computational graph :/'
                ...
                ber = self._get_ber(matrix_g, watermark.cuda())
                _, ber_ = self._extract(act, y_key_, watermarked_classes, matrix_a, watermark, config["architecture"])
            ...
            if (epoch + 1) % config["epoch_check"] == 0:
                with torch.no_grad():
                    ...
                    act = extractor(x_key_)[config["layer_name"]]
                    act = self._process_activation(act)
                    _, ber_ = self._extract(act, y_key_, watermarked_classes, matrix_a, watermark,
                                            config["architecture"])
    ...
    def extract(self, classifier, supp):
        # classifier.eval()
        extractor = create_feature_extractor(classifier, [supp["layer_name"]])
        x_key, y_key = next(iter(supp["key_loader"]))
        act = extractor(x_key)[supp["layer_name"]]
        # Get architecture from supplementary info if available, otherwise infer from activation shape
        architecture = supp.get("architecture", "MLP" if len(act.shape) == 2 else "CNN")
        # print("architecture", architecture)
        return self._extract(act, y_key, supp["watermarked_classes"], supp["matrix_a"], supp["watermark"], architecture)

    def _extract(self, act, y_key, watermarked_classes, matrix_a, watermark, architecture):
        # Process activation based on architecture
        act = self._process_activation(act)
    ...
    def _gmm(act, config):
        ...
        hist_gmm_classes = torch.histc(gmm_classes.float(), bins=config["n_components"], min=0,
                                       max=config["n_components"])
